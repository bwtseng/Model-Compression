...
dataloaders['train'] = torch.utils.data.DataLoader(train_dataset, batch_size=16, shuffle=True, num_workers=4, pin_memory=True)
dataloaders['val'] = torch.utils.data.DataLoader(val_dataset, batch_size=16, shuffle=True, num_workers=4, pin_memory=True)
dataloaders['test'] = torch.utils.data.DataLoader(test_dataset, batch_size=16, shuffle=True, num_workers=4, pin_memory=True)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model_path', default='model_save', type=str, help='Path to trained model.')
    parser.add_argument('--compress', default='yaml_file/vgg16.schedule_agp.yaml', type=str, help='Path to compress configure file.')
    parser.add_argument('--arch', default='alexnet', type=str, help='Name of used Architecture.')
    parser.add_argument('--name', default='alexnet_saved', type=str, help='Save file name.')
    ...
    if args.cpu: 
        device = torch.device("cpu")
        net = vgg16._vgg('vgg16', 'D', True, pretrained=args.pre_trained)
        net.arch = args.arch
        ...
        #net = alex.alexnet(pretrained=args.pre_trained)#.to(device)
        net = vgg16._vgg('vgg16', 'D', True, pretrained=args.pre_trained)
        #device = 'cuda'
        """
        if parallel:
        ...
        # Below line will wrap the sequential model into mobule type.
        ...
        if transfer_learning:
            ...
            pass
        # *****************************************************   
        # How to get intermediate layers using pytroch framework?
        # Basically, the layers are often defineds in the Sequential module
        # Thus, we need to get the name of this sequential and regard them as a list
        # and get the wieghts and feature just like tensorflow.
        # *****************************************************
        num_ftrs = net.module.classifier[-1].in_features
    ...
    if args.train:
        ...
        #print("\nTest wether successfully loading the model or not.\n")
