...
import model_zoo.resnet as rst
import summary 
...
if __name__ == '__main__':
    ...
    parser.add_argument('--compress', default='yaml_file/resnet50.schedule_agp.yaml', type=str, help='Path to compress configure file.')
    ...
    if args.cpu: 
        device = torch.device("cpu")
        net = rst.resnet50(pretrained=args.pre_trained)
        net.arch = args.arch
        ...
        net = rst.resnet50(pretrained=args.pre_trained)#.to(device)
        #device = 'cuda'
        """
        if parallel:
