...
# Should I add batchnornalization?
class Fire(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.concat(self.expand1x1_activation(self.expand1x1(x)),
    ...
    def __init__(self, version='1_0', num_classes=10):
        ...
        if version == '1_0':
            self.features = nn.Sequential(
                nn.Conv2d(3, 96, kernel_size=7, stride=2, padding=(2,2)),
...
def create_squeezenet(arch, pretrained, *args, **kwargs):
    if arch == 'squeezenet1_0':
        model = squeezenet1_0(pretrained)
    elif arch == 'squeezenet1_1':
        model = squeezenet1_1(pretrained)
    elif arch == 'squeezenet1_2':
        model = squeezenet1_2(pretrained)
    else:
        raise  ValueError('Not support this kind of squeezenet model !!!')
    return model
...
def _squeezenet(version, pretrained, progress, **kwargs):
    ...
    if pretrained:
        ...
        """
        raise ValueError("No pretrained models for squeezenet on CIFAR-10 dataset.")
    return model
