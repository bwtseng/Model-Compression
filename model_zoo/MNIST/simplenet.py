...
__all__ = ['simplenet_mnist', 'simplenet_v2_mnist', 'create_mnistnet']
...
def create_mnistnet(arch, pretrained=False, **kwargs):
    if arch == 'simplenet':
        model = simplenet_mnist()
    elif arch == 'simplenet_v1':
        model = simplenet_v2_mnist(pretrained)
    else:
        raise ValueError('Not support this kind of Alexnet model !!!')
    return model
