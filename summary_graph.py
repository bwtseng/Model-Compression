...
class SummaryGraph(object):
        ...
        for op in self.ops.values():
            ...
            if op['type'] == 'Conv':
                ...
                try:
                    ...
                    print("An input to a Convolutional layer is missing shape information"
                                    "(MAC values will be wrong)")
                    print("For details see https://github.com/NervanaSystems/distiller/issues/168")
                    op['attrs']['MACs'] = 0
            elif op['type'] == 'Gemm':
                conv_out = op['outputs'][0]
                conv_in = op['inputs'][0]
                try:
                    ...
                    print("An input to a Convolutional layer is missing shape information.")
                    print("For details see https://github.com/NervanaSystems/distiller/issues/360")
                    n_ifm = n_ofm = 0
    ...
    def find_op(self, lost_op_name):
        return self.ops.get(utility.normalize_module_name(lost_op_name), None)
    ...
    def predecessors(self, node, depth, done_list=None, denorm_names=True):
        ...
        if denorm_names:
            ret = [utility.denormalize_module_name(self._src_model, x) for x in ret]
        return ret
    ...
    def predecessors_f(self, node_name, predecessors_types, done_list=None, logging=None, denorm_names=True):
        ...
        """
        node_name = utility.normalize_module_name(node_name)
        ...
        if node_is_an_op:
            ...
            if node['type'] in predecessors_types and len(done_list) > 1:
                return [utility.denormalize_module_name(self._src_model, node_name) if denorm_names else node_name]
        ...
        ret = []
        for predecessor in preds:
            # Use recursive function may sacrifice the performance.
    ...
    def successors_f(self, node_name, successors_types, done_list=None, logging=None, denorm_names=True):
        ...
        if node is None:
            ...
            node = self.find_param(node_name)
            if node is None:
                print("successors_f: Could not find node {}".format(node_name))
                return []
    ...
    def named_params_layers(self):
        for param_name, param in self._src_model.named_parameters():
            ...
            normalized_layer_name = utility.normalize_module_name(
                '.'.join(param_name.split('.')[:-1]))
            sgraph_layer_name = utility.denormalize_module_name(
    ...
    def _dedicated_module_check(self, n, dedicated_modules_only=False):
        ...
        return len(self.module_ops_map[module_name]) == 1 and not utility.has_children(module)
    ...
    def adjacency_map(self, dedicated_modules_only=False):
        ...
        for op_name, op in self.ops.items():
            ...
            def op_meta(n):
                return OpSimpleMetadata(utility.denormalize_module_name(self._src_model, n), self.ops[n]['type'])
